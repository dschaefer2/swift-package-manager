...
# Extending Plugins for Custom Targets
## Requirements
...
- Need to be able to handle targets that don't have any sources but do have files generated by build plugin tools.
    - These targets do not have a path property. Need to make that optional. Setting to AbsolutePath.root for now.
- Need new module type for CustomModule for these targets with no sources
